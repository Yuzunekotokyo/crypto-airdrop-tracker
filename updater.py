...
def run_daily_update(force_email: bool = False) -> dict:
    ...
    # メール送信 (新着・変更・削除・ホット案件・強制送信)
    should_email = force_email or diff["added"] or newly_hot or diff["changed"] or diff["removed"]
    if should_email:
        sent = send_daily_report(new_airdrops, scraped_new, trending, diff=diff)
